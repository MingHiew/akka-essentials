...
class AskPec extends TestKit(ActorSystem("AskPec"))
...
object AskPec{
  ...
  class KVActor extends Actor with ActorLogging {
    ...
    class AuthManager extends Actor with ActorLogging {
      ...
      def handleAuthentication(user: String, password: String) = {
  ...
  class PipeAuthManager extends AuthManager {
    import AuthManager._
    override def handleAuthentication(user: String, password: String): Unit = {
      // step 3 - ask the actor
      val future = authDb ? Read(user) // Future[Any]
